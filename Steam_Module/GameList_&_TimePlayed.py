...
from ParentControlEmail import email_parent
...
for profile_link in myresult:
    ...
    # MySQL select Start
    mycursor = mydb.cursor()
    sql = "select count(*) from user_steam_games where user_id =" + str(profile_link[0])
    mycursor.execute(sql)
    ...
    # MySQL select Start
    mycursor = mydb.cursor()
    sql = "select id from user_steam_games where user_id =" + str(profile_link[0]) + " and game_id=3910"
    mycursor.execute(sql)
    game_list = mycursor.fetchall()
    print(game_list)
    # MySQL select Stop
    ...
    # MySQL select Start
    mycursor = mydb.cursor()
    sql = "select id from user_steam_games where user_id =" + str(profile_link[0]) + " and game_id=1546320"
    mycursor.execute(sql)
    game_list = mycursor.fetchall()
    print(game_list)
    # MySQL select Stop
    ...
    if time_played_old[0] is not None:
        time_played_difference = 11 #time_played_new[0] - time_played_old[0]
        if (time_played_difference > profile_link[3]) and (profile_link[3] > 0):
            email_parent(profile_link[0], time_played_difference)
    # Automated sending Email if played hours are above limit START
